...
def correlate(
    conversation_summary: str, rules_list: list[str], userResponse: str
) -> List[str]:
...
def correlation_matcher(conversation_summary: str, rules: list[str], userResponse: str):
    """
...
def parseEmotionVals(emotion_value_pair):
    output = ""
    for emp in emotion_value_pair:
        output += " " + str(emp)

    return "(" + output.strip() + ")"


def visualizeEmotionValues(
    *emotions: Any, scale_min: float = 0.0, scale_max: float = 1.0
):
    """
