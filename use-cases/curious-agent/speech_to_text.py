...
def pick_input_device(p: pyaudio.PyAudio):
    ...
    try:
            ...
            try:
                ...
                if inf.get("maxInputChannels", 0) > 0:
                    chosen_rate = (
                        int(env_rate)
                        if env_rate
                        else int(inf.get("defaultSampleRate", 16000))
                    )
        ...
        for idx, name, rate in map(info_tuple, candidates):
            lname = str(name).lower()
            if any(
                k in lname for k in ["microphone", "usb", "headset", "realtek", "alc"]
            ):
                return idx, rate
...
class SpeechToTextEngine:
    ...
    def __init__(
        ...
        try:
            ...
            logger.info(
                f"Found {len(device_candidates)} input devices, trying in priority order..."
            )

            # Try devices with simplified approach
            for dev_idx, dev_info in device_priority[
                :3
            ]:  # Limit to first 3 devices to prevent crashes
                try:
                    logger.info(
                        f"Trying device {dev_idx} ({dev_info.get('name', 'Default')})"
                    )

                    with ALSAErrorSuppressor():
                        # Use default sample rate and minimal configuration
                        mic = sr.Microphone(
                            device_index=dev_idx, sample_rate=16000, chunk_size=1024
                        )
                        ...
                        logger.info(
                            f"Success! Using device {dev_idx} ({dev_info.get('name', 'System Default')}) at 16000 Hz, threshold 300"
                        )
                        self.microphone = mic
                        break
        ...
        if not self.microphone:
            ...
            logger.warning(
                "No microphone found through validation, trying fallback approach..."
            )
            try:
                with ALSAErrorSuppressor():
                ...
                raise RuntimeError(
                    "No usable microphone found. Check audio setup, permissions, and devices."
                )
    ...
    def record_audio(
        self, duration: float = 5.0, output_file: Optional[str] = None
    ) -> str:
        """
...
class InteractiveSTT:
    ...
    def get_speech_input(self, prompt: str = "Speak now:") -> Optional[str]:
        return self.stt_engine.listen_once(
            timeout=10000000.0, phrase_time_limit=1000000.0
        )
        ...
        try:
            while True:
                text = self.stt_engine.listen_once(
                    timeout=100.0, phrase_time_limit=100.0
                )
                if text and on_text:
                    try:
                        on_text(text)
    ...
    try:
        ...
        for i in range(p.get_device_count()):
            ...
            if info.get("maxInputChannels", 0) > 0:
                ...
                print(
                    f"  🎤 Input {i}: {info['name']} (channels: {info['maxInputChannels']}, rate: {info['defaultSampleRate']})"
                )
...
if __name__ == "__main__":
    print(
        "This module provides SpeechToTextEngine and InteractiveSTT. Import and use from your agent."
    )
